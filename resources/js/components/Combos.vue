<template>
    <section id="combos-bio">
					...
					<div class="col-12">
						<div id="combo-group-1" class="row combo-group">
							<div class="row combos-on-mobile">
								<div class="col-6 col-lg-4" v-for="com in combo_product.slice(0,8)" :key="com.id">
									<div class="product-block">
										<div class="product-img">
											<img v-if="com.photo != null" :src="'storage/'+JSON.parse(com.photo)[0] | MediumImage">
											<div class="product-actions">
												<button type="button" class="btn" @click="addToCart(com,1)">
												...
												<button type="button" @click="getProduct(com)" class="btn" data-toggle="modal" data-target="#ModalProd">
										...
										<div class="product-prices" v-if="com.impuesto > 0">
											<p>IVA INCLUIDO</p>
										</div>
										<div class="product-prices" v-if="com.impuesto > 0">
											<p> ${{ (com.calculado / tasadolar) | FormatDolar}} / Bs {{ com.calculado | FormatNumber }}</p>
										</div>
										<div class="product-prices" v-if="!com.impuesto">
											<p> ${{ (com.price / tasadolar) | FormatDolar}} / Bs {{ com.price | FormatNumber }}</p>
										</div>
									</div>
								</div>
							</div>
					...
					<div id="right-combos" class="col-lg-12">
						<div class="row">
							<div id="combo-group-2" class="combo-group">
								<div class="col-6 col-lg-3" v-for="com in combo_product.slice(0,4)" :key="com.id">
									<div class="product-block">
										<div class="product-img">
											<img v-if="com.photo != null" :src="'storage/'+JSON.parse(com.photo)[0] | MediumImage">
											<div class="product-actions">
							...
							<div id="combo-group-3" class="combo-group">
								<div class="col-6 col-lg-3" v-for="com in combo_product.slice(4,8)" :key="com.id">
									<div class="product-block">
										<div class="product-img">
											<img v-if="com.photo != null" :src="'storage/'+JSON.parse(com.photo)[0] | MediumImage">
											<div class="product-actions">
												...
												<button type="button" @click="getProduct(com)" class="btn" data-toggle="modal" data-target="#ModalProdCombo">
													<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15.03 15"><title>visualizar-producto-bio</title><g id="Capa_2" data-name="Capa 2"><g id="Guias_y_recursos" data-name="Guias y recursos"><path class="cls-1" d="M5.77,11.54A5.77,5.77,0,0,1,1.69,1.69,5.77,5.77,0,0,1,9.85,9.85,5.74,5.74,0,0,1,5.77,11.54ZM5.77.77A5,5,0,1,0,9.3,2.23,5,5,0,0,0,5.77.77Z"/><path class="cls-1" d="M14.64,15a.4.4,0,0,1-.27-.11l-5.06-5a.37.37,0,0,1,0-.54.39.39,0,0,1,.55,0l5.06,5a.39.39,0,0,1,0,.55A.39.39,0,0,1,14.64,15Z"/></g></g></svg>
												</button>
											</div>
										</div>
		...
		<ModalCombo :combo="combo"></ModalCombo>
	</section>
</template>
<script>
